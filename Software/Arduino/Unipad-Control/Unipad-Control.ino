...
// unipad app:https://play.google.com/store/apps/details?id=jp.co.digitalray.unipad
// https://apps.apple.com/tr/app/unipad-remote-controller-unity-node-js-osc-websocket/id1055608061?l=tr
#ifndef APSSID
#define APSSID "wb-sct"
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t lenght)
{
  ...
  else if (type == WStype_CONNECTED)
  {
    ...
    WS_num = num;
    webSocket.sendTXT(num, "/config/type 1");
  }
...
void dc_drive(String dc_cmd) {
  String dc_direction ;
  int dc_speed = 255 ;
  int indexS = dc_cmd.indexOf('[');
  int indexE = dc_cmd.indexOf(']');



  dc_direction =  dc_cmd.substring(indexS + 2, indexE - 1);
  ...
  } else if (dc_direction == "Right") {
    analogWrite(A1, dc_speed - 130);
    analogWrite(A2, 0);
    analogWrite(B1, dc_speed - 150);
    analogWrite(B2, 0);
    analogWrite(C1, dc_speed - 150);
    analogWrite(C2, 0);
    analogWrite(D1, dc_speed - 150);
    analogWrite(D2, 0);
  } else if (dc_direction == "Left") {
    analogWrite(A1, 0);
    analogWrite(A2, dc_speed - 150);
    analogWrite(B1, 0);
    analogWrite(B2, dc_speed - 150);
    analogWrite(C1, 0);
    analogWrite(C2, dc_speed - 150);
    analogWrite(D1, 0);
    analogWrite(D2, dc_speed - 150);
  }
  if (indexS == indexE - 1) {
    digitalWrite(16, LOW);
    analogWrite(A1, 0);
